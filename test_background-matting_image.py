...
def init_model(model_dir: str) -> nn.Module:
    model_name = os.listdir(os.path.join("Models", model_dir))[0]
    model_params = os.path.join("Models", model_dir, model_name)
    netM = ResnetConditionHR(
        input_nc=(3, 3, 1, 4), output_nc=4, n_blocks1=7, n_blocks2=3
    )
    netM = nn.DataParallel(netM)
    netM.load_state_dict(torch.load(model_params))
    netM.cuda()
    netM.eval()
...
if __name__ == "__main__":
    ...
    # initialize network
    netM = init_model(args.trained_model)
    # input reoslution to the network
    input_res = (512, 512)
    ...
    test_imgs = glob.glob(os.path.join(args.input_dir, "*_img.png"))
    test_imgs.sort()

    if not os.path.exists(args.output_dir):
        os.makedirs(args.output_dir)

    for idx, image_path in enumerate(test_imgs):
        ...
        if args.back is None:
            ...
            bg_im0 = cv2.imread(image_path.replace("_img", "_back"))
            bg_im0 = cv2.cvtColor(bg_im0, cv2.COLOR_BGR2RGB)

        # segmentation mask
        rcnn = cv2.imread(image_path.replace("_img", "_masksDL"), 0)

        # if video mode, load target background frames
        if args.video:
            # target background path
            back_img10 = cv2.imread(
                os.path.join(args.target_back, image_name.replace("_img.png", ".png"))
            )
            back_img10 = cv2.cvtColor(back_img10, cv2.COLOR_BGR2RGB)
            # Green-screen background
            back_img20 = np.zeros(back_img10.shape)
            ...
            multi_frame_idx = [idx - 2 * gap, idx - gap, idx + gap, idx + 2 * gap]
            for multi_idx, frame_idx in enumerate(multi_frame_idx):
                if frame_idx < 0:
                    frame_idx = len(test_imgs) + frame_idx
                elif frame_idx >= len(test_imgs):
                    frame_idx = frame_idx - len(test_imgs)

                file_tmp = os.path.split(test_imgs[frame_idx])[-1]
                bgr_img_mul = cv2.imread(os.path.join(args.input_dir, file_tmp))
                multi_fr_w[..., multi_idx] = cv2.cvtColor(bgr_img_mul, cv2.COLOR_BGR2GRAY)
        ...
        crop_list = crop_images(crop_list, input_res, bbox)
        (bgr_img, bg_im, rcnn, back_img1, back_img2, multi_fr) = crop_list
        ...
        rcnn = np.delete(rcnn, range(input_res[0], input_res[0] + K), 0)
        ...
        write_image = lambda x, y: cv2.imwrite(os.path.join(args.output_dir, x), y)
        write_image(image_name.replace("_img", "_out"), alpha_out0)
        write_image(
            image_name.replace("_img", "_fg"), cv2.cvtColor(fg_out0, cv2.COLOR_BGR2RGB)
        )
        write_image(
            image_name.replace("_img", "_compose"),
            cv2.cvtColor(comp_im_tr1, cv2.COLOR_BGR2RGB),
        )
        write_image(
            image_name.replace("_img", "_matte").format(idx),
            cv2.cvtColor(comp_im_tr2, cv2.COLOR_BGR2RGB),
        )

        print(f"Done: {idx + 1} / {len(test_imgs)}", flush=True, end="\r")
