...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--model",
        type=str,
        default="xception_coco_voctrainval",
        choices=[
    ...
    list_im = glob.glob(os.path.join(dir_name, "*_img.png"))

    for idx, file_name in enumerate(list_im):
        ...
        name = file_name.replace("img", "masksDL")
        cv2.imwrite(name, (255 * mask_sel).astype(np.uint8))
        print(f"Done: {idx + 1} / {len(list_im)}", flush=True, end='\r')
